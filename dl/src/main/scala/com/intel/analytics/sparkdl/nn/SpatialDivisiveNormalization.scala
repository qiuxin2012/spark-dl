...
class SpatialDivisiveNormalization[T: ClassTag](
  ...
  if (kdim == 2) {
    ...
    meanestimator.add(new SpatialConvolutionMap[T](SpatialConvolutionMap.oneToOne[T](nInputPlane), kernel.size(1), 1))
  ...
  val stdestimator = new Sequential[Tensor[T], Tensor[T], T]()
  ...
  if (kdim == 2) {
    ...
    stdestimator.add(new SpatialConvolutionMap[T](SpatialConvolutionMap.oneToOne[T](nInputPlane), kernel.size(1), 1))
  ...
  // other operation
  val normalizer = new CDivTable()
  val divider = new CDivTable()
  val thresholder = new Threshold(threshold, thresval)

  // coefficient array, to adjust side effects
  var coef: Tensor[T] = Tensor(1,1,1)

  @transient private var ones: Tensor[T] = null
  @transient private var adjustedstds: Tensor[T] = null
  @transient private var thresholdedstds: Tensor[T] = null
  @transient private var localstds: Tensor[T] = null

  override def updateOutput(input: Tensor[T]): Tensor[T] = {
    localstds = stdestimator.updateOutput(input)

    // compute side coefficients
    val dim = input.dim()
    if (localstds.dim() != coef.dim() || (input.size(dim) != coef.size(dim)) || (input.size(dim-1) != coef.size(dim-1)) ) {
      ...
      if (dim == 4) {
        ...
        coef = meanestimator.updateOutput(ones)
      }
    }
  ...
  override def updateGradInput(input: Tensor[T], gradOutput: Tensor[T]): Tensor[T] = {
    ...
    gradInput.add(stdestimator.updateGradInput(input, graddiv(1)))
